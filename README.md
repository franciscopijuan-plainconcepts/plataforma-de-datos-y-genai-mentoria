...
> **Status**: v2.1 — Baseline + Text-to-SQL + Semantic Layer (with RLS governance) + Metabase visualization.
> The warehouse, data dictionary, NL→SQL pipeline, governed Semantic Layer,
> and Metabase integration (governed SQL cards from chat sessions) are all
> delivered. See `specs/` for the feature specs and `README_STATUS.md` for the
> current roadmap (milestones M0–M3.1 delivered).
...
### Metabase Integration (M3.1 / v2.1)

- A local Metabase instance (Docker, alongside PostgreSQL) that visualizes
  the SQL queries generated by the `ask` pipeline as cards/dashboards.
- Setup is automated and reproducible from a clean clone: `metabase setup`
  bootstraps the admin user, PostgreSQL DB connection (via a `metabase_readonly`
  role for defense-in-depth), and a `Chat Sessions` collection via REST API.
- Every successful `ask --viewer <id>` automatically creates a Metabase card
  with the **already-governed SQL** (Principle IV NON-NEGOTIABLE preserved by
  design — re-executing the card from Metabase returns only the viewer's
  regional scope). Use `--no-metabase` to skip; `--session <id>` groups cards
  into a `Session: <id>` dashboard.
- CLI: `metabase setup|status|teardown|reset-cards`.
...
- For Metabase integration (optional but enabled by default in `ask`):
  set `METABASE_HOST`, `METABASE_PORT`, `METABASE_ADMIN_EMAIL`, and
  `METABASE_ADMIN_PASSWORD` in `.env` (see `.env.example`).
  Run `metabase setup` once to bootstrap the Metabase container + admin user
  + DB connection + "Chat Sessions" collection.
...
# 7. (Optional) Set up Metabase and visualize queries as cards:
uv run python -m src.cli.main metabase setup
# Then every `ask` automatically creates a Metabase card in the 'Chat Sessions'
# collection with the ALREADY-GOVERNED SQL. Open http://localhost:3000 to view.
# Group several asks into a dashboard with: ask ... --session my-review
# Skip Metabase for a single ask: ask ... --no-metabase

# 8. Tear down the environment when done
